...
def ocr(path):
	...
	for word_info in extracted_data:
		...
		# Check if the current word is adjacent to the previous word
		if current_key and (abs(current_key[-1][3] - x1) <= 60 and abs(current_key[-1][2] - y1) <= 3) :
			...
			if len(current_key)>0:
				...
				merged_keys[merged_key] = {
				"word":merged_key,
				"coordinates":merged_coordinates
				}
				# print(current_key)
				# print(merged_keys[merged_key])	
	...
	for i in merged_keys.values():
		x1,y1,x2,y2=i['coordinates'][0],i['coordinates'][1],i['coordinates'][2],i['coordinates'][3] 
		cv2.rectangle(image, (x1, y1), (x2, y2), (0, 255, 0), 2)  # Draw a green rectangle
		# Put text label near the box
		cv2.putText(image, i['word'], (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
